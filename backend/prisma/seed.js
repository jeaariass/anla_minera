...
const CONFIG_TITULOS = [
  {
    ...
    minerales: ['Oro', 'Plata'],
    mesesActivos: [0, 1, 2, 3, 4],   // todos los meses
    metodo: 'Subterráneo',
  },
  {
  ...
  },
  {
    ...
    nit: null, nombreTitular: null, cedulaTitular: null,
    lat: 5.0646, lon: -75.8839,
    minerales: ['Arenas', 'Gravas'],
    mesesActivos: [0, 1, 2, 3, 4],   // todos los meses
    metodo: 'Aluvial',
    // Fecha exacta del INSERT original
    fechaInicio:      new Date('2020-01-01T00:00:00Z'),
    fechaVencimiento: new Date('2030-12-31T00:00:00Z'),
...
function zonaPorCategoria(cfg, categoria) {
  const OFFSETS = {
    extraccion:    { dLat:  0.012, dLon:  0.000, scatter: 0.0015 },
    acopio:        { dLat:  0.002, dLon:  0.015, scatter: 0.0015 },
    procesamiento: { dLat: -0.010, dLon:  0.005, scatter: 0.0012 },
  };
...
const AREAS_PROD   = ['Zona de Explotación A', 'Zona de Explotación B', 'Galería Principal'];
...
const NOMBRES_OP = [
  'Juan Pérez',      'Luis Gómez',       'Pedro Ramírez',   'Marcos Silva',
  'Oscar Díaz',      'Rodrigo Muñoz',    'Sebastián Castro', 'Alejandro Ríos',
  'Daniel Suárez',   'Héctor Ortiz',     'Miguel Pinto',    'David Torres',
  'Jorge Vega',      'Andrés León',      'Felipe Mora',
];
...
const ITEMS_DEF = [
  ...
  { categoria: 'procesamiento', codigo: 'PRO_001', nombre: 'Planta de Beneficio'      },
  { categoria: 'procesamiento', codigo: 'PRO_002', nombre: 'Piscina de Sedimentación' },
  { categoria: 'procesamiento', codigo: 'PRO_003', nombre: 'Tolva de Alimentación'    },
];

const CLIENTES_DEF = [
  { id: 'cliente-001', cedula: '12345678', nombre: 'Metales del Norte S.A.S'        },
  { id: 'cliente-002', cedula: '87654321', nombre: 'Comercializadora Aurífera Ltda'  },
  ...
  { id: 'cliente-006', cedula: '22334455', nombre: 'Inversiones Mineras del Caribe'  },
  ...
  { id: 'cliente-009', cedula: '77889900', nombre: 'Grupo Extractivo del Pacífico'   },
  { id: 'cliente-010', cedula: '44556677', nombre: 'Compañía Aurífera Central S.A'  },
];
...
async function main() {
  ...
  for (const t of CONFIG_TITULOS) {
    await prisma.tituloMinero.upsert({
      ...
      create: {
        ...
        nit: t.nit || null, nombreTitular: t.nombreTitular || null, cedulaTitular: t.cedulaTitular || null,
        estado: 'ACTIVO',
  ...
  for (const u of [
    ...
    await prisma.usuario.upsert({ where: { email: u.email }, update: {},
  ...
  for (let i = 0; i < CONFIG_TITULOS.length; i++) {
    await prisma.usuario.upsert({ where: { email: `titular${i+1}@tumina.co` }, update: {},
      create: { id: `user-titular-00${i+1}`, nombre: `Titular ${CONFIG_TITULOS[i].municipio}`,
        email: `titular${i+1}@tumina.co`, password: pwd, rol: 'TITULAR', activo: true,
  ...
  for (let i = 0; i < CONFIG_TITULOS.length; i++) {
    await prisma.usuario.upsert({ where: { email: `jefe${i+1}@tumina.co` }, update: {},
      create: { id: JEFE_IDS[i], nombre: `Jefe Planta ${CONFIG_TITULOS[i].municipio}`,
        email: `jefe${i+1}@tumina.co`, password: pwd, rol: 'JEFE_PLANTA', activo: true,
        tituloMineroId: TITULO_IDS[i] } });
  }
  ...
  for (let t = 0; t < CONFIG_TITULOS.length; t++) {
    ...
    for (let o = 0; o < OPS_POR_TITULO[t]; o++) {
      ...
      ids.push(id);
      await prisma.usuario.upsert({ where: { email: `op.t${t+1}.${o+1}@tumina.co` }, update: {},
        create: { id, nombre: `${NOMBRES_OP[o % NOMBRES_OP.length]} (T${t+1})`,
          email: `op.t${t+1}.${o+1}@tumina.co`, password: pwd, rol: 'OPERARIO', activo: true,
  ...
  for (const m of MOTIVOS_DEF) {
    await prisma.paradas_motivos.upsert({ where: { codigo: m.codigo }, update: {},
  ...
  for (const item of ITEMS_DEF) {
    try {
      await prisma.puntos_items_catalogo.upsert({
        ...
        update: {}, create: { ...item, activo: true, orden: 0 },
      });
    } catch (_) {}
  }
  ...
  for (const m of MAQ_DEF) {
    try {
      await prisma.maquinaria_catalogo.upsert({ where: { codigo: m.codigo }, update: {},
  ...
  for (const c of CLIENTES_DEF) {
    try {
      await prisma.clientes_compradores.upsert({ where: { cedula: c.cedula }, update: {},
  ...
  const totalFRI = { produccion:0, inventarios:0, paradas:0, ejecucion:0, maquinaria:0, regalias:0, invMaq:0, capacidad:0, proyecciones:0 };
    ...
    for (const idxMes of cfg.mesesActivos) {
      ...
      for (const mineral of cfg.minerales) {
        ...
        await prisma.fRIInventarios.create({ data: {
          ...
          inventarioInicialAcopio: invInicial, ingresoAcopio: ingreso, salidaAcopio: salida,
          inventarioFinalAcopio: parseFloat((invInicial + ingreso - salida).toFixed(4)),
          estado, tituloMineroId: cfg.id, usuarioId: jefeId,
        }});
        ...
        await prisma.fRIEjecucion.create({ data: {
          ...
          latitud:  parseFloat((cfg.lat  + (Math.random()-0.5)*0.005).toFixed(8)),
          longitud: parseFloat((cfg.lon  + (Math.random()-0.5)*0.005).toFixed(8)),
          metodoExplotacion: cfg.metodo,
          avanceEjecutado: rand(10, 90, 2), unidadMedidaAvance: 'Metros',
        ...
        await prisma.fRIRegalias.create({ data: {
          fechaCorte, mineral,
          cantidadExtraida: rand(400, 2000, 4), unidadMedida: 'Toneladas',
      ...
      const finP = new Date(diaP); finP.setHours(diaP.getHours() + rand(4, 16));
      await prisma.fRIParadas.create({ data: {
        fechaCorte, tipoParada: pick(TIPOS_PARADA), fechaInicio: diaP, fechaFin: finP,
        horasParadas: rand(4, 24, 2), motivo: pick(MOTIVOS_DEF).nombre,
        estado, tituloMineroId: cfg.id, usuarioId: jefeId,
      }});
      totalFRI.paradas++;

      await prisma.fRIMaquinaria.create({ data: {
        fechaCorte, tipoMaquinaria: pick(TIPOS_MAQ), cantidad: rand(2, 8),
        horasOperacion: rand(80, 220, 2), capacidadTransporte: rand(15, 50, 2),
      ...
      await prisma.fRIInventarioMaquinaria.create({ data: {
        fechaCorte, tipoMaquinaria: pick(TIPOS_MAQ),
        marca: pick(MAQ_DEF).marca, modelo: pick(MAQ_DEF).modelo,
        a_oFabricacion: rand(2015, 2023), capacidad: rand(20, 60, 2),
      ...
      await prisma.fRICapacidad.create({ data: {
        fechaCorte, areaProduccion: pick(AREAS_PROD), tecnologiaUtilizada: cfg.metodo,
        capacidadInstalada: rand(1000, 5000, 4), unidadMedida: 'Toneladas/Mes',
        personalCapacitado: rand(5, 30), certificaciones: `ISO-9001, NTC-${rand(1000, 9999)}`,
        estado, tituloMineroId: cfg.id, usuarioId: jefeId,
      }});
      totalFRI.capacidad++;

      await prisma.fRIProyecciones.create({ data: {
        fechaCorte, metodoExplotacion: cfg.metodo, mineral: cfg.minerales[0],
  ...
  for (let t = 0; t < CONFIG_TITULOS.length; t++) {
    ...
    for (const idxMes of cfg.mesesActivos) {
      ...
      for (let p = 0; p < nParadas; p++) {
        ...
        const topeP  = (año === hoyRef.getFullYear() && mes === hoyRef.getMonth())
          ? hoyRef.getDate() : 26;
        ...
        await prisma.paradas_actividad.create({ data: {
          ...
          inicio, fin, dia: new Date(año, mes, dia),
  ...
  for (let t = 0; t < CONFIG_TITULOS.length; t++) {
    ...
    for (const idxMes of cfg.mesesActivos) {
      ...
      for (let c = 0; c < nCerts; c++) {
        ...
        const topeC    = (año === hoyRef2.getFullYear() && mes === hoyRef2.getMonth())
          ? hoyRef2.getDate() : 27;
  ...
  CONFIG_TITULOS.forEach(c => {
    console.log(`   ${c.numeroTitulo.padEnd(10)} ${c.municipio.padEnd(12)} | ${c.minerales.join(', ').padEnd(25)} | ${c.mesesActivos.length} meses`);
  });
  ...
  console.log(`   Operarios       : ${totalOps}  (${OPS_POR_TITULO.join(' / ')} por título)`);
  console.log(`   TOTAL           : ${3+5+5+5+totalOps}`);

  const totalMineralMes = CONFIG_TITULOS.reduce((acc, c) => acc + c.mesesActivos.length * c.minerales.length, 0);
  const totalMes        = CONFIG_TITULOS.reduce((acc, c) => acc + c.mesesActivos.length, 0);
  console.log('\n 📋 FORMULARIOS FRI:');
  console.log(`   Producción      : ${totalFRI.produccion}  (1 por mineral × mes activo)`);
  ...
  console.log('\n 🗺️  OPERACIÓN:');
  console.log(`   Puntos actividad  : ~${totalPuntos}  (10-12/día/título → 50-60 por día en total)`);
  console.log(`   Paradas actividad : ${totalParadasAct}  (3-5 por mes por título)`);
  console.log(`   Certificados      : ${certContador}  (4-6 por mes por título, multi-mineral y cliente)`);
