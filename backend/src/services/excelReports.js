// anla_minera/backend/src/services/excelReports.js
...
class ExcelReportService {
  ...
  async generarReporteProduccion(datos, filtros = {}) {
    ...
    datos.forEach((item, index) => {
      ...
      dataRow.getCell(1).value = new Date(item.fechaCorte).toLocaleDateString(
        "es-CO",
      );
      dataRow.getCell(2).value = sanitizarCelda(item.mineral);
      dataRow.getCell(3).value = sanitizarCelda(
        item.tituloMinero?.numeroTitulo || "N/A",
      );
      dataRow.getCell(4).value = sanitizarCelda(
        item.tituloMinero?.municipio || "N/A",
      );
      dataRow.getCell(5).value = parseFloat(item.horasOperativas);
      dataRow.getCell(6).value = parseFloat(item.cantidadProduccion);
      dataRow.getCell(7).value = sanitizarCelda(item.unidadMedida);
    ...
    totalRow.getCell(5).value = datos.reduce(
      (sum, item) => sum + parseFloat(item.horasOperativas),
      0,
    );
    totalRow.getCell(6).value = datos.reduce(
      (sum, item) => sum + parseFloat(item.cantidadProduccion),
      0,
    );
  ...
  async generarReporteConsolidado(datos) {
    ...
    tiposFormulario.forEach((tipo) => {
      if (tipo.datos && tipo.datos.length > 0) {
        ...
        summarySheet.getCell(`C${row}`).value = tipo.datos.filter(
          (d) => d.estado === "BORRADOR",
        ).length;
        summarySheet.getCell(`D${row}`).value = tipo.datos.filter(
          (d) => d.estado === "ENVIADO",
        ).length;
        row++;
      }
