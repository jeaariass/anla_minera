...
function resolverRuta(numeroTitulo, consecutivo, ext) {
  ...
  const relativa = path
    .join(tituloCarpeta, yyyy, mm, nombre)
    .replace(/\\/g, "/");
...
function listarArbol(tituloFiltro = null) {
  if (!fs.existsSync(STORAGE_ROOT)) return [];

  const titulos = fs.readdirSync(STORAGE_ROOT).filter((f) => {
    if (!fs.statSync(path.join(STORAGE_ROOT, f)).isDirectory()) return false;
    if (!tituloFiltro) return true;
    const carpeta = f.toUpperCase().replace(/[^A-Z0-9]/g, "");
    const filtro = tituloFiltro.toUpperCase().replace(/[^A-Z0-9]/g, "");
    return carpeta === filtro;
  });

  return titulos.map((titulo) => {
    const tituloPath = path.join(STORAGE_ROOT, titulo);
    const años = fs
      .readdirSync(tituloPath)
      ...
      .map((anio) => {
        ...
        const meses = fs
          .readdirSync(path.join(tituloPath, anio))
          .filter((f) =>
            fs.statSync(path.join(tituloPath, anio, f)).isDirectory(),
          )
