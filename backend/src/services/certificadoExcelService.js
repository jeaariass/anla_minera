...
const TEMPLATE_PATH = path.join(__dirname, "../templates/certificado_origen.xlsx");
const FIRMA_PATH    = path.join(__dirname, "../templates/firma.png");

function dato(ws, coord, valor) {
  ws.getCell(coord).value = (valor !== null && valor !== undefined && valor !== "") ? valor : null;
}
...
async function generarCertificadoExcel(datos) {
  const { id, fechaCertificado, mineralExplotado, cantidadM3, unidadMedida, titulo, cliente } = datos;
  ...
  dato(ws, "C4", String(fecha.getDate()).padStart(2, "0"));
  dato(ws, "D4", String(fecha.getMonth() + 1).padStart(2, "0"));
  dato(ws, "E4", String(fecha.getFullYear()));
  ...
  marcarX(ws, {
    "NIT": "F10", "CÉDULA": "G10",
    "CÉDULA DE EXTRANJERÍA": "H10", "RUT": "I10",
  }, "CÉDULA");
  ...
  marcarX(ws, {
    "NIT": "F19", "CÉDULA": "G19",
    "CÉDULA DE EXTRANJERÍA": "H19", "RUT": "I19",
