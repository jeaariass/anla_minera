// anla_minera/backend/src/services/pdfExporter.js
// ============================================
...
class PDFExporter {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (datosPorTipo.puntosActividad) {
          if (!esPrimeraPagina) doc.addPage();
          this.generarPaginaPuntosActividad(
            doc,
            datosPorTipo.puntosActividad,
            filtros,
          );
          esPrimeraPagina = false;
        }
  ...
  generarPortada(doc, filtros) {
    ...
    doc
      ...
      .fillColor("#475569")
      .font("Helvetica")
      .text("Formato de Reporte de Información", 0, 220, {
        width: pageWidth,
        align: "center",
      });
  ...
  dibujarEncabezado(doc, titulo, color, filtros = {}) {
    ...
    if (filtros.fechaInicio || filtros.fechaFin) {
      ...
      doc.text(`Período: ${periodo}`, rightX, 30, {
        width: 240,
        align: "right",
      });
    }
  ...
  generarTabla(doc, headers, rows, startY) {
    ...
    rows.forEach((row, rowIndex) => {
      ...
      row.forEach((cell, i) => {
        doc.fillColor("#334155").text(String(cell), 40 + i * colWidth, y, {
          width: colWidth - 4,
          align: "center",
