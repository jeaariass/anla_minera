...
const buscarCliente = async (req, res) => {
  try {
    ...
    if (!cedula && !correo)
      return res
        .status(400)
        .json({ success: false, message: "Debes enviar cedula o correo." });
    ...
    // NUEVO — H-02: si el cliente existe pero no tiene relación con
...
const crearCliente = async (req, res) => {
  try {
    ...
    if (!cedula || !nombre)
      return res
        .status(400)
        .json({ success: false, message: "Cédula y nombre son obligatorios." });
    ...
    if (!tipoComprador)
      return res.status(400).json({
        success: false,
        message: "El tipo de comprador es obligatorio.",
      });

    const existe =
      await prisma.$queryRaw`SELECT id FROM clientes_compradores WHERE cedula = ${String(cedula).trim()} LIMIT 1`;
...
const actualizarCliente = async (req, res) => {
  try {
    ...
    // Solo se puede editar un cliente con el que el usuario
    // ya tiene una relación (un certificado emitido a su
    // propio título minero). Los roles globales (ADMIN/ASESOR)
...
const descargarExcel = async (req, res) => {
  try {
    ...
    const { fullPath: xlsxPath, relativa: xlsxRel } = resolverRuta(
      titulo?.numeroTitulo || "SIN_TITULO",
      consecutivo,
      "xlsx",
    );
...
const listarCertificados = async (req, res) => {
  try {
    ...
    const where = tituloMineroId
      ? `WHERE co."tituloMineroId" = '${tituloMineroId}'`
      : "";
