    ...
    switch (tipo) {
      case "produccion":
        return {
          ...
          Cantidad_material_entra_Plantabeneficio: formatearNumero(
            registro.materialEntraPlanta,
          ),
          Cantidad_material_sale_Plantabeneficio: formatearNumero(
            registro.materialSalePlanta,
          ),
      ...
      case "inventarios":
        return {
          ...
          Inventario_Inicial_Acopio: formatearNumero(
            registro.inventarioInicialAcopio,
          ),
          Inventario_Final_Acopio: formatearNumero(
            registro.inventarioFinalAcopio,
          ),
      ...
      case "ejecucion":
        return {
          ...
          Avance_Ejecutado:
            registro.avanceEjecutado != null
              ? Number(registro.avanceEjecutado)
              : "",
          Unidad_medida_avance: registro.unidadMedidaAvance || "",
          Volumen_Ejecutado:
            registro.volumenEjecutado != null
              ? Number(registro.volumenEjecutado)
              : "",
          Estado: base.Estado,
        };
      ...
      case "regalias":
        return {
          ...
          Valor_Contraprestaciones: formatearNumero(
            registro.valorContraprestaciones,
          ),
      ...
      case "paradasActividad":
        return {
          ...
          Fin: registro.fin
            ...
            : "",
          Minutos_paro:
            registro.minutos_paro != null ? Number(registro.minutos_paro) : "",
      ...
      case "certificadosOrigen":
        return {
          ...
          Cliente_Nombre:
            registro.clientes_compradores?.nombre || registro.clienteId || "",
          Cliente_Cedula: registro.clientes_compradores?.cedula || "",
          Cantidad_M3:
            registro.cantidadM3 != null ? Number(registro.cantidadM3) : 0,
...
exports.getPreview = async (req, res) => {
  try {
    ...
    if (!tipo) {
      return res
        .status(400)
        .json({ success: false, message: "Tipo de formulario requerido" });
    }
    ...
    if (tipo === "certificadosOrigen") {
      try {
        ...
        else if (!esGCert && decoded.tituloMineroId)
          filtroCert.tituloMineroId = decoded.tituloMineroId;

        if (fechaInicio || fechaFin) {
          filtroCert.fechaCertificado = {};
          if (fechaInicio)
            filtroCert.fechaCertificado.gte = new Date(fechaInicio);
          if (fechaFin)
            filtroCert.fechaCertificado.lte = new Date(fechaFin + "T23:59:59");
        }
        if (mineral && mineral !== "")
          filtroCert.mineralExplotado = {
            contains: mineral,
            mode: "insensitive",
          };
        ...
        const paramsPrev = [];
        let pp = 1;
        if (filtroCert.tituloMineroId) {
          wherePrev.push(`c."tituloMineroId" = $${pp++}`);
          paramsPrev.push(filtroCert.tituloMineroId);
        }
        if (filtroCert.clienteId) {
          wherePrev.push(`c."clienteId" = $${pp++}`);
          paramsPrev.push(filtroCert.clienteId);
        }
        if (filtroCert.fechaCertificado?.gte) {
          wherePrev.push(`c."fechaCertificado" >= $${pp++}`);
          paramsPrev.push(filtroCert.fechaCertificado.gte);
        }
        if (filtroCert.fechaCertificado?.lte) {
          wherePrev.push(`c."fechaCertificado" <= $${pp++}`);
          paramsPrev.push(filtroCert.fechaCertificado.lte);
        }
        if (filtroCert.mineralExplotado?.contains) {
          wherePrev.push(`c."mineralExplotado" ILIKE $${pp++}`);
          paramsPrev.push("%" + filtroCert.mineralExplotado.contains + "%");
        }
        ...
        const datosCertPrev = rawPrev.map((r) => ({
          ...r,
          cantidadM3: r.cantidadM3 ?? 0,
          clientes_compradores: {
            nombre: r.cliente_nombre || "",
            cedula: r.cliente_cedula || "",
          },
        }));

        const columnasPrev = getColumnas("certificadosOrigen");
        const registrosPrev = transformarDatos(
          datosCertPrev,
          "certificadosOrigen",
        );
        return res.json({
          success: true,
          columnas: columnasPrev,
          registros: registrosPrev,
          total: registrosPrev.length,
        });
      } catch (certErr) {
        console.error("[cert preview] error:", certErr.message);
        return res
          .status(500)
          .json({ success: false, message: certErr.message });
      }
    }
    ...
    const campoFecha =
      tipo === "puntosActividad"
        ? "fecha"
        : tipo === "paradasActividad"
          ? "inicio"
          : "fechaCorte";
  ...
  try {
    ...
    if (tipo === "certificadosOrigen") {
      try {
        ...
        if (esGCertExp && tituloPCExp)
          filtroCertExp.tituloMineroId = tituloPCExp;
        else if (!esGCertExp && decoded.tituloMineroId)
          filtroCertExp.tituloMineroId = decoded.tituloMineroId;

        if (fechaInicio || fechaFin) {
          filtroCertExp.fechaCertificado = {};
          if (fechaInicio)
            filtroCertExp.fechaCertificado.gte = new Date(fechaInicio);
          if (fechaFin)
            filtroCertExp.fechaCertificado.lte = new Date(
              fechaFin + "T23:59:59",
            );
        }
        if (mineral && mineral !== "")
          filtroCertExp.mineralExplotado = {
            contains: mineral,
            mode: "insensitive",
          };
        ...
        const paramsExp = [];
        let pe = 1;
        if (filtroCertExp.tituloMineroId) {
          whereExp.push(`c."tituloMineroId" = $${pe++}`);
          paramsExp.push(filtroCertExp.tituloMineroId);
        }
        if (filtroCertExp.clienteId) {
          whereExp.push(`c."clienteId" = $${pe++}`);
          paramsExp.push(filtroCertExp.clienteId);
        }
        if (filtroCertExp.fechaCertificado?.gte) {
          whereExp.push(`c."fechaCertificado" >= $${pe++}`);
          paramsExp.push(filtroCertExp.fechaCertificado.gte);
        }
        if (filtroCertExp.fechaCertificado?.lte) {
          whereExp.push(`c."fechaCertificado" <= $${pe++}`);
          paramsExp.push(filtroCertExp.fechaCertificado.lte);
        }
        if (filtroCertExp.mineralExplotado?.contains) {
          whereExp.push(`c."mineralExplotado" ILIKE $${pe++}`);
          paramsExp.push("%" + filtroCertExp.mineralExplotado.contains + "%");
        }
        ...
        wsCert.columns = colsCert.map((col) => ({
          header: col,
          key: col,
          width: 22,
        }));
        wsCert.getRow(1).font = { bold: true };
        wsCert.getRow(1).alignment = {
          vertical: "middle",
          horizontal: "center",
        };
        transformarDatos(datosCertExp, "certificadosOrigen").forEach((r) =>
          wsCert.addRow(Object.values(r)),
        );
        wsCert.eachRow((row) =>
          row.eachCell((cell) => {
            cell.border = {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="${nombreCertExp}"`,
        );
        ...
        return res
          .status(500)
          .json({ success: false, message: certExpErr.message });
      }
    }
    ...
    if (datos.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No hay datos para exportar" });
    }
