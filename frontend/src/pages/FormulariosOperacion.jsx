...
const DraggableMarker = ({ position, onMove }) => {
  ...
  return (
    ...
    >
      <Popup>
        <strong>📍 Arrastra para ajustar</strong>
        <br />
...
const FormulariosOperacion = () => {
  ...
  const cargarParadas = async () => {
    try {
      setLoadingP(true);
      const r = await api.get(`/paradas/${tituloActivoId}`);
      if (r.data.success) setParadas(r.data.data ?? []);
    } catch {
      showAlert(setAlertP, "error", "No se pudieron cargar las paradas");
    } finally {
      setLoadingP(false);
    }
  };
  ...
  const cargarPuntos = async () => {
    try {
      setLoadingPt(true);
      const r = await api.get(`/actividad/puntos/${tituloActivoId}`);
      if (r.data.success) setPuntos(r.data.data ?? []);
    } catch {
      showAlert(setAlertPt, "error", "No se pudieron cargar los puntos");
    } finally {
      setLoadingPt(false);
    }
  };
  ...
  const handleSubmitParada = async (e) => {
    e.preventDefault();
    if (!fp.motivoId)
      return showAlert(setAlertP, "error", "Selecciona un motivo");
    if (!fp.inicio || !fp.fin)
      return showAlert(setAlertP, "error", "Ingresa inicio y fin");
    ...
    try {
      ...
      if (r.data.success) {
        showAlert(
          ...
          editParada
            ? "✅ Paro actualizado"
            : `✅ Paro registrado · ${r.data.data?.minutosRegistrados ?? "?"} min`,
        );
        resetFormP();
        cargarParadas();
      } else {
  ...
  const handleDeleteParada = async (p) => {
    if (
      !window.confirm(
        `¿Eliminar el paro "${p.motivoNombre}"? Esta acción no se puede deshacer.`,
      )
    )
      return;
    try {
  ...
  const handleSubmitPunto = async (e) => {
    e.preventDefault();
    if (!fpt.categoria)
      return showAlert(setAlertPt, "error", "Selecciona una categoría");
    if (!fpt.itemId)
      return showAlert(setAlertPt, "error", "Selecciona un ítem");
    if (!fpt.latitud || !fpt.longitud)
      return showAlert(
        setAlertPt,
        "error",
        "Ingresa las coordenadas o usa el botón GPS",
      );
    ...
    try {
      ...
      const payload = {
        ...
        maquinariaOtro:
          maquinaria.find((m) => m.id === fpt.maquinariaId)?.codigo === "OTRO"
            ? fpt.maquinariaOtro.trim()
            : null,
      ...
      if (r.data.success) {
        showAlert(
          setAlertPt,
          "success",
          editPunto ? "✅ Punto actualizado" : "✅ Punto registrado",
        );
        resetFormPt();
        cargarPuntos();
      } else {
  ...
  const handleDeletePunto = async (pt) => {
    if (
      !window.confirm(
        `¿Eliminar el punto "${pt.itemDisplay || pt.itemNombre || pt.categoria}"?`,
      )
    )
      return;
    try {
      ...
      !busqP.trim()
        ...
        : paradas.filter(
            (p) =>
              (p.motivoNombre || "")
                .toLowerCase()
                .includes(busqP.toLowerCase()) || (p.dia || "").includes(busqP),
          ),
    [paradas, busqP],
  );
        ...
        <div className="container">
          ...
          <div className="page-title-section" style={{ marginBottom: 28 }}>
            ...
            <div>
              ...
              <p className="page-subtitle">
                Registra, edita y elimina paradas y puntos de actividad
              </p>
            </div>
          </div>
          ...
          {tab === "paradas" && (
            <div className="fop-layout">
              ...
              <div className="fop-form-card">
                <div
                  className={`fop-form-header ${editParada ? "edit-header" : "paradas-header"}`}
                >
                ...
                <form className="fop-form-body" onSubmit={handleSubmitParada}>
                  {alertP && (
                    <div className={`fop-alert ${alertP.type}`}>
                      {alertP.msg}
                    </div>
                  )}
                  ...
                  <div className="fop-field">
                    ...
                    <select
                      ...
                      value={fp.motivoId}
                      onChange={(e) =>
                        setFp((p) => ({ ...p, motivoId: e.target.value }))
                      }
                      required
                    >
                  ...
                  {motiSel?.codigo === "OTRO" && (
                    <div className="fop-field">
                      ...
                      <input
                        ...
                        value={fp.motivoOtro}
                        onChange={(e) =>
                          setFp((p) => ({ ...p, motivoOtro: e.target.value }))
                        }
                      />
                    </div>
                  )}
                  ...
                  <div className="fop-row-2">
                    <div className="fop-field">
                      ...
                      <input
                        ...
                        value={fp.inicio}
                        onChange={(e) =>
                          setFp((p) => ({ ...p, inicio: e.target.value }))
                        }
                        required
                      />
                    </div>
                  ...
                  <div className="fop-field">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setFp((p) => ({ ...p, observaciones: e.target.value }))
                      }
                    />
                  </div>
              ...
              <div className="fop-list-card">
                ...
                {loadingP ? (
                  ...
                  <div className="fop-empty">
                    <span>🛑</span>
                    <p>
                      {busqP ? "Sin resultados" : "No hay paradas registradas"}
                    </p>
                  </div>
                ) : (
          ...
          {tab === "puntos" && (
            <div className="fop-layout">
              ...
              <div className="fop-form-card">
                <div
                  className={`fop-form-header ${editPunto ? "edit-header" : "puntos-header"}`}
                >
                  <MapPin size={20} />
                ...
                <form className="fop-form-body" onSubmit={handleSubmitPunto}>
                  {alertPt && (
                    <div className={`fop-alert ${alertPt.type}`}>
                      {alertPt.msg}
                    </div>
                  )}
                  ...
                  {fpt.categoria && (
                    <div className="fop-field">
                      ...
                      >
                        <option value="">
                          — {loadingItems ? "Cargando..." : "Seleccionar ítem"}{" "}
                          —
                        </option>
                        {items.map((i) => (
                      ...
                      {itemSel?.codigo === "OTRO" && (
                        <input
                          ...
                          value={fpt.itemOtro}
                          onChange={(e) =>
                            setFpt((p) => ({ ...p, itemOtro: e.target.value }))
                          }
                        />
                      )}
                    </div>
                  ...
                  <div className="fop-field">
                    ...
                    <input
                      ...
                      value={fpt.latitud}
                      onChange={(e) =>
                        setFpt((p) => ({ ...p, latitud: e.target.value }))
                      }
                    ...
                    <input
                      ...
                      value={fpt.longitud}
                      onChange={(e) =>
                        setFpt((p) => ({ ...p, longitud: e.target.value }))
                      }
                    />
                    <div className="fop-gps-buttons">
                      <button
                        type="button"
                        className="fop-gps-btn"
                        onClick={handleGPS}
                      >
                        📡 Usar GPS
                      </button>
                      <button
                        ...
                        title={
                          !fpt.latitud || !fpt.longitud
                            ? "Ingresa coordenadas primero"
                            : "Ver en mapa"
                        }
                      >
                        🗺️ Ver en mapa
                  ...
                  <div className="fop-row-2">
                    <div className="fop-field">
                      ...
                      <input
                        ...
                        value={fpt.volumenM3}
                        onChange={(e) =>
                          setFpt((p) => ({ ...p, volumenM3: e.target.value }))
                        }
                      />
                    </div>
                    <div className="fop-field">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFpt((p) => ({ ...p, descripcion: e.target.value }))
                        }
                      />
                    </div>
                  </div>
                ...
                {mapModalOpen &&
                  ...
                  (() => {
                    ...
                    const dispLat =
                      mapTempCoords?.lat ?? parseFloat(fpt.latitud);
                    const dispLng =
                      mapTempCoords?.lng ?? parseFloat(fpt.longitud);
                    const initCenter = [
                      parseFloat(fpt.latitud),
                      parseFloat(fpt.longitud),
                    ];
                    ...
                    return (
                      <div
                        className="fop-map-modal-overlay"
                        onClick={handleCancel}
                      >
                        <div
                          className="fop-map-modal"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <div className="fop-map-modal-header">
                            <h4>🗺️ Ajustar Ubicación</h4>
                            <button
                              className="fop-map-modal-close"
                              onClick={handleCancel}
                            >
                              ✕
                            </button>
                          ...
                          >
                            ✋ <strong>Arrastra el pin</strong> o{" "}
                            <strong>haz clic</strong> en el mapa para mover la
                            ubicación
                          </div>
                          ...
                          <div className="fop-map-modal-footer">
                            <button
                              className="fop-map-cancel-btn"
                              onClick={handleCancel}
                            >
                              Cancelar
                            </button>
                            <button
                              className="fop-map-confirm-btn"
                ...
                <div className="fop-cat-filters">
                  {[
                    ...
                    >
                      ...
                      {id === ""
                        ? puntos.length
                        : puntos.filter((p) => p.categoria === id).length}
                      )
                    </button>
                  ))}
                </div>
                        ...
                        {puntosFiltrados.map((pt) => {
                          ...
                          return (
                            <tr key={pt.id}>
                              ...
                              <td>
                                <span
                                  className={`fop-cat-badge ${pt.categoria}`}
                                >
