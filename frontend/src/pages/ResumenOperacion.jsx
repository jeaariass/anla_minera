...
const ResumenOperacion = () => {
  ...
  // const tituloMineroId =
  //   user?.tituloMinero?.id || user?.tituloMineroId || "titulo-816-17";
  ...
  const cargarPuntos = async () => {
    try {
      setLoadingPt(true);
      setErrorPt("");
      const res = await api.get(`/actividad/puntos/${tituloActivoId}`);
  ...
  const puntosFiltrados = useMemo(() => {
    return puntos.filter((p) => {
      ...
      const matchQ =
        !busqPt.trim() ||
        (p.itemDisplay || p.itemNombre || "")
          .toLowerCase()
          .includes(busqPt.toLowerCase()) ||
        (p.maquinariaNombre || "")
          .toLowerCase()
          .includes(busqPt.toLowerCase()) ||
  ...
  // ── KPIs ───────────────────────────────────────────────────────────────────
  const totalMinutos = paradas.reduce(
  ...
  const volTotal = puntos.reduce(
    (acc, p) => acc + (Number(p.volumenM3) || 0),
    0,
  );
          ...
          <div className="page-title-section" style={{ marginBottom: 28 }}>
            ...
            <div>
              ...
              <p className="page-subtitle">
                Paradas de actividad y puntos georeferenciados registrados desde
                la app móvil
              </p>
            </div>
          </div>
            ...
            >
              ...
              <span
                style={{
                  marginLeft: 4,
                  background:
                    tab === "paradas" ? "rgba(255,255,255,0.25)" : "#f1f5f9",
            ...
            >
              ...
              <span
                style={{
                  marginLeft: 4,
                  background:
                    tab === "puntos" ? "rgba(255,255,255,0.25)" : "#f1f5f9",
            ...
            <>
              ...
              <div className="kpi-grid">
                ...
                >
                  ...
                  <div className="kpi-data">
                    ...
                    <p style={{ fontSize: 24 }}>
                      {paradas.length > 0
                        ? Math.round(totalMinutos / paradas.length)
                        : 0}{" "}
                      min
                    </p>
              ...
              <div className="section-header">
                <div>
                  ...
                  <p>
                    Paros registrados desde la app móvil — ordenados por fecha
                    más reciente
                  </p>
                </div>
                <button
                ...
                >
                  <RefreshCw
                    size={16}
                    style={{
                      animation: loadingP ? "spin 1s linear infinite" : "none",
                    }}
                  />
                        ...
                        <tbody>
                          {paradasFiltradas.map((p, i) => (
                            <tr key={p.id}>
                              ...
                              <td>
                                <span
                                  className="motivo-badge"
                                  title={p.motivoNombre}
                                >
                                  {p.motivoDisplay ||
                                    p.motivoNombre ||
                                    p.motivoCodigo ||
              ...
              <div className="section-header">
                ...
                >
                  <RefreshCw
                    size={16}
                    style={{
                      animation: loadingPt ? "spin 1s linear infinite" : "none",
                    }}
                  />
