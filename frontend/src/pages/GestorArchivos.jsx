...
const GestorArchivos = () => {
  ...
  const cargarArbol = async () => {
    try {
      ...
      const res = await api.get("/archivos", {
        params: { tituloMineroId: tituloActivoId || undefined },
      });
  ...
  // ── Contar totales ────────────────────────────────────────────────────────
  const totalArchivos = arbol.reduce(
    (t, tit) =>
      t +
      tit.años.reduce(
        (t2, a) => t2 + a.meses.reduce((t3, m) => t3 + m.archivos.length, 0),
        0,
      ),
    0,
  );
        ...
        <div className="ga-toolbar">
          <div className="ga-toolbar-left">
            ...
            <div>
              ...
              <p>
                {arbol.length} título{arbol.length !== 1 ? "s" : ""} ·{" "}
                {totalArchivos} archivo{totalArchivos !== 1 ? "s" : ""}
              </p>
            </div>
          </div>
          <button
            className="ga-btn-refresh"
            onClick={cargarArbol}
            disabled={loading}
          >
            ...
            {arbol.map((tit) => {
              ...
              return (
                <div key={tit.titulo} className="ga-node ga-titulo">
                  {/* ── Título minero ── */}
                  <button
                    className="ga-node-header"
                    onClick={() => toggle(keyTit)}
                  >
                    {abierto ? (
                      <ChevronDown size={16} />
                    ) : (
                    ...
                    {abierto ? (
                      <FolderOpen size={20} className="ga-icon-titulo" />
                    ) : (
                      <Folder size={20} className="ga-icon-titulo" />
                    )}
                    ...
                    <span className="ga-node-count">
                      {tit.años.reduce(
                        (t, a) =>
                          t +
                          a.meses.reduce((t2, m) => t2 + m.archivos.length, 0),
                        0,
                      )}{" "}
                      archivos
                    </span>
                          ...
                          <div key={anio.anio} className="ga-node ga-anio">
                            ...
                            >
                              ...
                              {abiAnio ? (
                                <FolderOpen
                                  size={18}
                                  className="ga-icon-anio"
                                />
                              ) : (
                                <Folder size={18} className="ga-icon-anio" />
                              )}
                            ...
                            {abiAnio && (
                              <div className="ga-children">
                                {anio.meses.map((mes) => {
                                  ...
                                  const pdfs = mes.archivos.filter(
                                    (a) => a.ext === "pdf",
                                  );
                                  const xlsxs = mes.archivos.filter(
                                    (a) => a.ext === "xlsx",
                                  );
                                  ...
                                  return (
                                    <div
                                      key={mes.mes}
                                      className="ga-node ga-mes"
                                    >
                                      {/* ── Mes ── */}
                                      <div className="ga-mes-header">
                                        <button
                                          className="ga-node-header"
                                          onClick={() => toggle(keyMes)}
                                        >
                                          {abiMes ? (
                                            <ChevronDown size={14} />
                                          ) : (
                                        ...
                                        <button
                                          ...
                                          onClick={() =>
                                            descargarMes(
                                              ...
                                              anio.anio,
                                              mes.mes,
                                              `${MESES_NOMBRES[mes.mes]}_${anio.anio}`,
                                            )
                                          }
                                        >
                                          {descargando === zipKey ? (
                                      ...
                                      {abiMes && (
                                        <div className="ga-children ga-archivos">
                                          ...
                                          {mes.archivos.map((arch) => (
                                            <div
                                              key={arch.nombre}
                                              className="ga-archivo"
                                            >
                                              <div className="ga-archivo-info">
                                                {arch.ext === "pdf" ? (
                                                  <FileText
                                                    size={16}
                                                    className="ga-icon-pdf"
                                                  />
                                                ) : (
                                                  <FileSpreadsheet
                                                    size={16}
                                                    className="ga-icon-xlsx"
                                                  />
                                                )}
                                                <div>
                                                  <span className="ga-archivo-nombre">
                                                    {arch.nombre}
                                                  </span>
