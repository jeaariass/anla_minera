...
const DEFAULT_CENTER = [
  5.7, -75.5,
]; /* Eje Cafetero — Caldas / Risaralda / Antioquia */
...
/* ════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════ */
const ANM_WMS_URL =
  "https://geo.anm.gov.co/webgis/services/ANM/ServiciosANM/MapServer/WMSServer";
...
const TituloPoligonoMapa = ({ info }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Tooltip sticky>
          Título {info.id} · {info.municipio}, {info.departamento} ·{" "}
          {info.area_ha} ha
        </Tooltip>
      </GeoJSON>
...
const CATEGORIAS = [
  {
    ...
    label: "Gestión FRI",
    sublabel:
      "Formatos de Reporte según la Resolución 371/2024 de la Agencia Nacional de Mineria",
  ...
  {
    id: "operacion",
    label: "Actividades en Campo",
    sublabel: "Operación y paradas diarias en la mina",
    icon: <Wrench size={18} />,
    accentColor: "#e74c3c",
    bgLight: "#fff4f4",
    borderColor: "#fca5a5",
  ...
  {
    id: "certificados",
    label: "Certificados y Ventas",
    sublabel: "Origen mineral y documentos",
    icon: <Award size={18} />,
    accentColor: "#059669",
    bgLight: "#f0fdf4",
    borderColor: "#6ee7b7",
  },
  {
    ...
    sublabel: "Usuarios y control de acceso",
...
const Home = () => {
  ...
  const [tituloGeoData, setTituloGeoData] = useState(null);
  ...
  useEffect(() => {
    if (!tituloActivoId) {
      setTituloGeoData(null);
      return;
    }

    const fetchGeoData = async () => {
      try {
        const res = await api.get(`/titulos/${tituloActivoId}`);
        if (res.data.success && res.data.titulo) {
          const t = res.data.titulo;
          if (!t.centroid || !t.polygon) {
            setTituloGeoData(null);
            return;
          }
          ...
          setTituloGeoData({
            ...
            area_ha: t.area_ha ? String(t.area_ha).replace(".", ",") : "–",
            clasificacion: t.clasificacion || "–",
            minerales: t.minerales || "–",
            titular: t.nombreTitular || "–",
            modalidad: t.modalidad || "–",
            expedicion: t.fechaInicio
              ? new Date(t.fechaInicio).toLocaleDateString("es-CO", {
                  year: "numeric",
                  month: "short",
                  day: "numeric",
                })
              : "–",
            expiracion: t.fechaVencimiento
    ...
    try {
      ...
      if (tienePermiso("VER_FRI")) {
        for (const tipo of tipos) {
          try {
            ...
            const res = await friService[method]({
      ...
      try {
        ...
        if (rPuntos.data.success) {
          ...
          sd.puntosHoy = all.filter(
            (p) =>
              p.dia &&
              String(p.dia).split("T")[0] === hoy &&
  ...
  const quickActions = [
    /* FRI — tonos índigo */
    {
      ...
      description: "Crea, edita y envía tus formularios FRI",
      path: "/formularios",
      cardBg: "#4f46e5",
    ...
    },
    {
      ...
      title: "Estadísticas FRI",
      description: "Analiza el estado y avance de tus reportes",
      path: "/dashboard",
      cardBg: "#6366f1",
    ...
    },
    {
      ...
      description: "Descarga reportes consolidados en Excel",
      path: "/reportes",
      cardBg: "#818cf8",
    ...
    /* Operación — tonos rojo/coral */
    {
      ...
      description: "Reporta paradas y puntos de actividad del turno",
    ...
    },
    {
      categoria: "operacion",
      icon: <BarChart3 size={26} />,
      title: "Estadísticas de Operación",
      description: "Revisa gráficas de paradas y tiempos operados",
    ...
    },
    {
      categoria: "operacion",
      icon: <Map size={26} />,
      title: "Mapa de Actividades en Campo",
      description: "Consulta puntos registrados",
      path: "/mapa",
      cardBg: "#f87171",
    ...
    /* Certificados — tonos verde esmeralda */
    {
      ...
      description: "Genera y descarga certificados de origen",
    ...
    },
    {
      ...
      description: "Accede a certificados y documentos guardados",
    ...
    /* Admin — tonos azul marino */
    {
      ...
      description: "Administra cuentas, permisos y títulos",
      path: "/usuarios",
      cardBg: "#1e3a5f",
        ...
        <div className="container">
                    ...
                    {tituloActivo && (
                      <div className="wchip wchip--purple">
                        ...
                        {tituloActivo.municipio && (
                          <span>· {tituloActivo.municipio}</span>
                        )}
                      </div>
                    )}
                    <div className="wchip">
                  ...
                  <div className="titulo-header-left">
                    ...
                    <div>
                      <h3 className="titulo-card-title">
                        Título Minero &nbsp;
                        <span className="titulo-badge">{tituloGeoData.id}</span>
                      </h3>
                      <p className="titulo-card-sub">
                        {tituloGeoData.municipio}, {tituloGeoData.departamento}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="titulo-ficha">
                    <div className="titulo-ficha-grid">
                      <div className="titulo-campo">
                        ...
                        <span className="titulo-estado-chip">
                          {tituloGeoData.estado}
                        </span>
                      </div>
                      <div className="titulo-campo">
                        <span className="titulo-campo-label">Etapa</span>
                        <span className="titulo-campo-val titulo-campo-val--etapa">
                          {tituloGeoData.etapa}
                        </span>
                      </div>
                      <div className="titulo-campo titulo-campo--wide">
                        <span className="titulo-campo-label">Modalidad</span>
                        <span className="titulo-campo-val">
                          {tituloGeoData.modalidad}
                        </span>
                      </div>
                      <div className="titulo-campo">
                        <span className="titulo-campo-label">Área</span>
                        <span className="titulo-campo-val">
                          <strong>{tituloGeoData.area_ha}</strong> ha
                        </span>
                      </div>
                      <div className="titulo-campo">
                        <span className="titulo-campo-label">
                          Clasificación
                        </span>
                        <span className="titulo-campo-val">
                          {tituloGeoData.clasificacion}
                        </span>
                      </div>
                      <div className="titulo-campo titulo-campo--wide">
                        <span className="titulo-campo-label">Minerales</span>
                        <span className="titulo-campo-val">
                          {tituloGeoData.minerales}
                        </span>
                      </div>
                      <div className="titulo-campo titulo-campo--wide">
                        <span className="titulo-campo-label">Titular</span>
                        <span className="titulo-campo-val">
                          {tituloGeoData.titular}
                        </span>
                      </div>
                      <div className="titulo-campo">
                        <span className="titulo-campo-label">Expedición</span>
                        <span className="titulo-campo-val">
                          {tituloGeoData.expedicion}
                        </span>
                      </div>
                      <div className="titulo-campo">
                        <span className="titulo-campo-label">
                        ...
                        <span className="titulo-campo-val titulo-campo-val--vigencia">
                          {tituloGeoData.expiracion}
                        </span>
                      </div>
                    </div>
                    <div className="titulo-anm-badge">
                      <span>🛰</span>
                      Información tomada del Visor Geográfico Oficial de la
                      Agencia Nacional de Minería
                    </div>
                  </div>
                  <div className="titulo-mapa-wrap">
                    <TituloPoligonoMapa info={tituloGeoData} />
                    <div className="titulo-mapa-footer">
                      <MapPin size={11} />
                      Título <strong>{tituloGeoData.id}</strong> · Centroide:{" "}
                      {tituloGeoData.centroid[1].toFixed(5)},{" "}
                      {tituloGeoData.centroid[0].toFixed(5)}
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="fri-panel">
                  <div className="fri-panel-header">
                    <div className="fri-panel-header-left">
                      ...
                      <div>
                        <h3 className="fri-panel-title">
                          Estado de tus reportes FRI
                        </h3>
                        <p className="fri-panel-subtitle">
                          Período activo · Título{" "}
                  ...
                  <div className="fri-metrics-row">
                    <div className="fri-metric-hero">
                      <div className="fri-metric-hero-icon">
                        <FileText size={28} />
                      </div>
                      <div>
                        <div className="fri-metric-hero-value">
                          {stats.totalFormularios}
                        </div>
                        <div className="fri-metric-hero-label">
                          formulario{stats.totalFormularios !== 1 ? "s" : ""} en
                          total
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="fri-estado-trio">
                      <div className="fri-estado fri-estado--draft">
                        <span className="fri-estado-icon">
                          <Edit size={15} />
                        </span>
                        <span className="fri-estado-num">
                          {stats.borradores}
                        </span>
                    ...
                    {stats.totalFormularios > 0 && (
                      <div className="fri-donut-wrap">
                        ...
                        <div className="fri-donut-track">
                          <div
                            className="fri-donut-seg seg-draft"
                            style={{ flex: stats.borradores }}
                            title={`Borradores: ${stats.borradores}`}
                          />
                          <div
                            className="fri-donut-seg seg-sent"
                            style={{ flex: stats.enviados }}
                          ...
                          {stats.totalFormularios -
                            ...
                            0 && (
                            <div
                              className="fri-donut-seg seg-other"
                              style={{
                                flex:
                                  stats.totalFormularios -
                                  stats.borradores -
                  ...
                  <div className="op-body op-body--solo">
                    ...
                    <div className="stat-op stat-op--blue">
                      ...
                      <div className="stat-op-body">
                        ...
                        <div className="stat-op-label">
                          Puntos de actividad hoy
                        </div>
                        <div className="stat-op-sub">
                          Georeferenciados en campo
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <section className="actions-section">
            <h3 className="section-title">
              <Zap size={20} strokeWidth={2.5} /> ¿Qué vas a hacer hoy?
            </h3>
                ...
                >
                  <div className="category-header">
                    ...
                    <div className="category-header-text">
                      ...
                      {cat.sublabel && (
                        <span className="category-sublabel">
                          {cat.sublabel}
                        </span>
                      )}
                    </div>
