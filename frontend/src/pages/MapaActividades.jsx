...
import {
  ArrowLeft, User, LogOut, Map as MapIcon, Filter,
  FileSpreadsheet, Download, ChevronUp, ChevronDown, ChevronsUpDown,
  Clock, AlertTriangle,
} from "lucide-react";
...
const MapaActividades = () => {
  ...
  const cargarPuntos = async () => {
    try {
      setLoading(true);
      if (!tituloId) { setLoading(false); return; }
      const res = await api.get(`/actividad/puntos/${tituloId}`);
      if (res.data.success) {
        setTodosLosPuntos(res.data.data);
        if (res.data.data.length > 0) {
          const fp = res.data.data[0];
          setFirstCenter([parseFloat(fp.latitud), parseFloat(fp.longitud)]);
        }
      }
  ...
  const cargarParadas = async () => {
    try {
      ...
      if (!tituloId) { setLoadingParadas(false); return; }
      const res = await api.get(`/paradas/${tituloId}`);
      if (res.data.success) setTodasParadas(res.data.data);
  ...
  const usuariosPuntos  = useMemo(() => [...new Set(todosLosPuntos.map(p => p.usuarioId).filter(Boolean))].sort(), [todosLosPuntos]);
  const usuariosParadas = useMemo(() => [...new Set(todasParadas.map(p => p.usuarioId).filter(Boolean))].sort(), [todasParadas]);
  ...
  const paradasFiltradas = useMemo(() => todasParadas.filter(p => {
    if (rUsuario  && p.usuarioId !== rUsuario) return false;
  ...
  const handleExportarPuntos = async () => {
    try {
      ...
      const res  = await reportService.exportarExcel({ tipo: "puntosActividad", fechaInicio: pFechaInicio, fechaFin: pFechaFin });
      const blob = new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
      const url  = URL.createObjectURL(blob);
      const a    = document.createElement("a");
      const nombre = `Actividades_${pCategoria || "todas"}_${new Date().toISOString().split("T")[0]}.xlsx`;
      Object.assign(a, { href: url, download: nombre }); document.body.appendChild(a); a.click(); a.remove();
      URL.revokeObjectURL(url);
      setExportSuccessPuntos(`✓ "${nombre}" descargado`);
    } catch (err) { setExportErrorPuntos(err.response?.data?.message || "Error al exportar"); }
  ...
  const handleExportarParadas = async () => {
    try {
      ...
      const res  = await reportService.exportarExcel({ tipo: "paradasActividad", fechaInicio: rFechaInicio, fechaFin: rFechaFin });
      const blob = new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
      const url  = URL.createObjectURL(blob);
      const a    = document.createElement("a");
    ...
    } catch (err) { setExportErrorParadas(err.response?.data?.message || "Error al exportar"); }
              ...
              <div className="form-grid" style={{ marginBottom:16 }}>
                ...
                <div className="form-group">
                  ...
                  <select className="form-control" value={pUsuario} onChange={e => setPUsuario(e.target.value)}>
                    ...
                    {usuariosPuntos.map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </div>
                <div className="form-group">
              ...
              <div className="form-grid" style={{ marginBottom:16 }}>
                <div className="form-group">
                  ...
                  <select className="form-control" value={rUsuario} onChange={e => setRUsuario(e.target.value)}>
                    <option value="">Todos</option>
                    {usuariosParadas.map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label>📊 Estado</label>
