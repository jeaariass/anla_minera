...
const SortIcon = ({ col, sortCol, sortDir }) => {
  if (sortCol !== col)
    return (
      <ChevronsUpDown
        size={12}
        style={{ opacity: 0.35, marginLeft: 3, verticalAlign: "middle" }}
      />
    );
  return sortDir === "asc" ? (
    ...
    <div className="tabla-wrapper">
      ...
      {datos.length > maxRows && (
        <div className="tabla-info">
          Mostrando {maxRows} de {datos.length} registros. Exporta a CSV para
          ver todos.
        </div>
      )}
    </div>
...
const MapaActividades = () => {
  ...
  useEffect(() => {
    usuarioService
      .getAll()
      .then((r) => {
        if (r.data?.success) setUsuarios(r.data.usuarios || []);
      })
  ...
  useEffect(() => {
    ...
    cargarPuntos();
    setPUsuario(""); // ← resetear filtro de usuario
  }, [tituloActivoId]);
  useEffect(() => {
    if (!tituloActivoId) return;
    cargarParadas();
    setRUsuario(""); // ← resetear filtro de usuario
  }, [tituloActivoId]);
  ...
  const filtrarPuntos = () => {
    if (!filtroCategoria) setPuntosFiltrados(todosLosPuntos);
      ...
      setPuntosFiltrados(
        todosLosPuntos.filter((p) => p.categoria === filtroCategoria),
      );
  };
  ...
  const contarPorCategoria = (cat) =>
    !cat
      ...
      : todosLosPuntos.filter((p) => p.categoria === cat).length;

  // ── Usuarios únicos para los selects ──────────────────
  // Usuarios únicos que tienen puntos en el título activo
  const usuariosPuntos = useMemo(() => {
    const ids = [
      ...new Set(todosLosPuntos.map((p) => p.usuarioId).filter(Boolean)),
    ];
    return usuarios.filter((u) => ids.includes(u.id));
  }, [todosLosPuntos, usuarios]);

  // Usuarios únicos que tienen paradas en el título activo
  const usuariosParadas = useMemo(() => {
    const ids = [
      ...new Set(
        todasParadas.map((p) => p.usuarioId || p.usuario_id).filter(Boolean),
      ),
    ];
    return usuarios.filter((u) => ids.includes(u.id));
  }, [todasParadas, usuarios]);

  // ── Datos filtrados puntos ─────────────────────────────
  const puntosFiltradosReporte = useMemo(
    () =>
      todosLosPuntos.filter((p) => {
        if (pCategoria && p.categoria !== pCategoria) return false;
        if (pUsuario && p.usuarioId !== pUsuario) return false;
        if (pFechaInicio && new Date(p.fecha) < new Date(pFechaInicio))
          return false;
        if (pFechaFin && new Date(p.fecha) > new Date(pFechaFin + "T23:59:59"))
          return false;
        return true;
      }),
  ...
  const paradasFiltradas = useMemo(() => {
    console.log(
      "🔍 Filtrando paradas | total:",
      todasParadas.length,
      "| rUsuario:",
      rUsuario,
    );
    if (todasParadas.length > 0)
      console.log("  ejemplo usuarioId:", todasParadas[0].usuarioId);
  ...
  const totalMinutosParadas = useMemo(
    () =>
      paradasFiltradas.reduce(
        (acc, p) => acc + (Number(p.minutesParo) || 0),
        0,
      ),
    [paradasFiltradas],
  );
  ...
  const handleExportarPuntos = async () => {
    try {
      setExportandoPuntos(true);
      setExportErrorPuntos("");
      setExportSuccessPuntos("");
      const params = {
      ...
      if (res.data instanceof Blob && res.data.type?.includes("json")) {
        ...
        throw new Error(JSON.parse(text).message || "Error");
      }
      ...
      setExportErrorPuntos(
        err.message || err.response?.data?.message || "Error al exportar",
      );
    } finally {
      setExportandoPuntos(false);
    }
  };
  ...
  const handleExportarParadas = async () => {
    try {
      setExportandoParadas(true);
      setExportErrorParadas("");
      setExportSuccessParadas("");
      const params = {
        tipo: "paradasActividad",
        fechaInicio: rFechaInicio,
        fechaFin: rFechaFin,
        usuarioId: rUsuario || "",
        estado: rEstado || "",
        tituloMineroId: tituloActivoId || "",
  ...
  const renderPunto = (p, key) => {
    switch (key) {
      case "fecha":
      case "updated_at":
        return <span style={{ whiteSpace: "nowrap" }}>{fmtFecha(p[key])}</span>;
      case "usuarioId":
        return <span className="chip-usuario">{p.usuarioId || "—"}</span>;
      case "categoria":
        return (
          <span
            style={{
      ...
      case "volumenM3":
        return p.volumenM3 != null ? `${p.volumenM3} m³` : "—";
      case "descripcion":
        return (
          <span
            style={{
      ...
      <main className="reportes-main">
        <div className="container">
          ...
          <div className="page-title-section">
            ...
            <div>
              ...
              <p className="page-subtitle">
                Visualización georeferenciada — Mostrando:{" "}
          ...
          <div className="card" style={{ marginBottom: "20px" }}>
            ...
            >
              <MapContainer
                center={firstCenter || [5.0689, -75.5174]}
                zoom={firstCenter ? 10 : 8}
                        ...
                        <div style={{ minWidth: "200px" }}>
                          ...
                          {punto.itemDisplay && (
                            <p style={{ margin: "5px 0" }}>
                              <strong>Actividad:</strong> {punto.itemDisplay}
                            </p>
                          )}
                          {punto.descripcion && (
                            <p style={{ margin: "5px 0" }}>
                          ...
                          >
                            📍 {parseFloat(punto.latitud).toFixed(6)},{" "}
                            {parseFloat(punto.longitud).toFixed(6)}
                          </p>
                          <p
                            style={{
                              margin: "5px 0 0 0",
          ...
          <div className="card" style={{ marginBottom: "20px" }}>
            <div className="card-header">
              ...
              <h3 style={{ margin: 0, flex: 1 }}>
                ...
                {puntosFiltradosReporte.length > 0 && (
                  <span className="badge-filtros" style={{ marginLeft: 10 }}>
                    {puntosFiltradosReporte.length} registros
                  </span>
                )}
              </h3>
            </div>
            <div className="card-body">
              ...
              <div className="form-grid" style={{ marginBottom: 16 }}>
                ...
                <div className="form-group">
                  ...
                  <select
                    className="form-control"
                    value={pUsuario}
                    onChange={(e) => setPUsuario(e.target.value)}
                  >
                    <option value="">Todos</option>
                    {usuariosPuntos.map((u) => (
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    className="form-control"
                    value={pFechaFin}
                    onChange={(e) => setPFechaFin(e.target.value)}
                    min={pFechaInicio || undefined}
                  />
                </div>
                <div
                ...
                >
                  ...
                  <button
                    ...
                    style={{ height: 40 }}
                    onClick={handleExportarPuntos}
                    disabled={
                      exportandoPuntos || puntosFiltradosReporte.length === 0
                    }
                  >
                    {exportandoPuntos ? (
                      <>
              ...
              {puntosFiltradosReporte.length === 0 ? (
                ...
                <TablaReporte
                  datos={puntosFiltradosReporte}
                  columnas={COLS_PUNTOS}
                  renderCell={renderPunto}
                />
              )}
            </div>
          </div>
          ...
          <div className="card" style={{ marginBottom: "20px" }}>
            <div className="card-header">
              ...
              <h3 style={{ margin: 0, flex: 1 }}>
                ...
                {paradasFiltradas.length > 0 && (
                  <span className="badge-filtros" style={{ marginLeft: 10 }}>
                    {paradasFiltradas.length} registros
                  </span>
                )}
              </h3>
              {/* Totalizador */}
            ...
            <div className="card-body">
              ...
              <div className="form-grid" style={{ marginBottom: 16 }}>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    className="form-control"
                    value={rFechaInicio}
                    onChange={(e) => setRFechaInicio(e.target.value)}
                    max={rFechaFin || undefined}
                  />
                </div>
                <div className="form-group">
                  <label>📅 Fecha Fin</label>
                  <input
                    ...
                    className="form-control"
                    value={rFechaFin}
                    onChange={(e) => setRFechaFin(e.target.value)}
                    min={rFechaInicio || undefined}
                  />
                </div>
                <div
                  className="form-group"
                ...
                >
                  ...
                  <button
                    className="btn btn-success"
                    style={{ height: 40 }}
                    onClick={handleExportarParadas}
                    disabled={
                      exportandoParadas || paradasFiltradas.length === 0
                    }
                  >
                    {exportandoParadas ? (
                      <>
              ...
              {loadingParadas ? (
                ...
                <TablaReporte
                  datos={paradasFiltradas}
                  columnas={COLS_PARADAS}
                  renderCell={renderParada}
                />
              )}
            </div>
          </div>
          ...
          <div className="card info-card">
            ...
            <div className="card-body">
              <ol className="instruction-list">
                <li>
                  Usa los botones de categoría para filtrar los marcadores en el
                  mapa
                </li>
                <li>
                  El mapa se centra automáticamente en el primer punto
                  registrado
                </li>
                <li>
                  Filtra el reporte de actividad por categoría, usuario y fechas
                </li>
                <li>
                  Filtra el reporte de paradas por usuario, estado y fechas
                </li>
                <li>
                  Haz clic en los encabezados de columna para ordenar la tabla
                </li>
                <li>
                  Exporta actividades a Excel o paradas a CSV (compatible con
                  Excel)
                </li>
              </ol>
            </div>
