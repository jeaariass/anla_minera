...
const CAT_LABELS = {
  extraccion: 'Extracción', acopio: 'Acopio',
  procesamiento: 'Procesamiento', inspeccion: 'Inspección',
};

const MOTIVO_COLORS = [
  '#667eea','#e74c3c','#f39c12','#27ae60','#3498db','#8b5cf6','#ec4899','#06b6d4',
];
...
const DashboardOperacion = () => {
  ...
  // ── KPIs ───────────────────────────────────────────────────────────────────
  const kpis = useMemo(() => {
    const totalMinutos  = paradasFiltradas.reduce((a, p) => a + (Number(p.minutesParo) || 0), 0);
    const totalVol      = puntosFiltrados.reduce((a, p)  => a + (Number(p.volumenM3)   || 0), 0);
    const diasConParada = new Set(paradasFiltradas.map(p => String(p.dia).split('T')[0])).size;
    return {
  ...
  const puntosPorDia = useMemo(() => {
    ...
    return Object.values(mapa).sort((a, b) => a.dia.localeCompare(b.dia))
      .map(r => ({ ...r, volumen: parseFloat(r.volumen.toFixed(2)), label: fmtDay(r.dia) }));
  }, [puntosFiltrados]);
  ...
  const categoriasDist = useMemo(() => {
    ...
    puntosFiltrados.forEach(p => {
      ...
      if (!mapa[k]) mapa[k] = { nombre: CAT_LABELS[k] || k, valor: 0, volumen: 0 };
      mapa[k].valor++;
      ...
      <main className="dopd-main">
        <div className="container">
          ...
          <div className="page-title-section" style={{ marginBottom: 28 }}>
            <div className="page-title-icon" style={{ background: 'linear-gradient(135deg,#e74c3c,#f39c12)' }}>
              <span style={{ fontSize: 36 }}>📊</span>
            </div>
            <div>
              <h2 className="page-title">Dashboard de Operación</h2>
              <p className="page-subtitle">Análisis de paradas y puntos de actividad registrados desde la app móvil</p>
            </div>
          </div>

          {/* ── Filtros de período ── */}
          <div className="dopd-filters">
            <label><Filter size={14} /> Desde:</label>
            <input
              type="date" className="dopd-date-input" value={desde}
              max={hasta}
              onChange={e => { setDesde(e.target.value); setPeriodo('custom'); }}
            />
            <label>Hasta:</label>
            <input
              type="date" className="dopd-date-input" value={hasta}
              min={desde} max={hoy}
              onChange={e => { setHasta(e.target.value); setPeriodo('custom'); }}
            />
            <button className="dopd-filter-btn" onClick={cargar} disabled={loading}>
          ...
          <div className="dopd-kpi-grid">
            ...
            <div className="dopd-kpi" style={{ borderLeftColor: '#e74c3c' }}>
              <div className="dopd-kpi-icon" style={{ background: '#fee2e2' }}>🛑</div>
              <div className="dopd-kpi-body">
                <h5>Total Paradas</h5>
                <strong>{kpis.totalParadas}</strong>
                <span>en {kpis.diasConParada} día{kpis.diasConParada !== 1 ? 's' : ''} con paro</span>
              </div>
            </div>

            <div className="dopd-kpi" style={{ borderLeftColor: '#f39c12' }}>
              <div className="dopd-kpi-icon" style={{ background: '#fef3c7' }}>⏱️</div>
              <div className="dopd-kpi-body">
                <h5>Tiempo Parado</h5>
                <strong>{kpis.horasParadas}h</strong>
                <span>{kpis.totalMinutos} min · {kpis.promedioMin} min/paro promedio</span>
              </div>
            </div>

            <div className="dopd-kpi" style={{ borderLeftColor: '#2563eb' }}>
              <div className="dopd-kpi-icon" style={{ background: '#dbeafe' }}>📍</div>
              <div className="dopd-kpi-body">
                <h5>Total Puntos</h5>
            ...
            <div className="dopd-kpi" style={{ borderLeftColor: '#8b5cf6' }}>
              <div className="dopd-kpi-icon" style={{ background: '#ede9fe' }}>📦</div>
              <div className="dopd-kpi-body">
                <h5>Volumen Total</h5>
          ...
          <div className="dopd-charts-grid">
            ...
            <div className="dopd-card">
              <div className="dopd-card-header">
                ...
                <span className="dopd-card-badge">{paradasPorDia.length} días</span>
              </div>
              {paradasPorDia.length > 0 ? (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={paradasPorDia} barGap={4}>
                    ...
                    <Bar yAxisId="left"  dataKey="paradas" fill="#e74c3c" name="Paradas"        radius={[4,4,0,0]} />
                    <Bar yAxisId="right" dataKey="minutos" fill="#fbbf24" name="Minutos parado" radius={[4,4,0,0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="dopd-no-data"><span>🛑</span><p>Sin paradas en el período</p></div>
              )}
            </div>
            ...
            <div className="dopd-card">
              <div className="dopd-card-header">
                ...
                <span className="dopd-card-badge">{motivosDist.length} motivos</span>
              </div>
              {motivosDist.length > 0 ? (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={motivosDist} layout="vertical" margin={{ left: 10 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 11 }} allowDecimals={false} />
                    <YAxis type="category" dataKey="nombre" tick={{ fontSize: 11 }} width={130} />
                    <Tooltip content={<CustomTooltip />} />
                    <Bar dataKey="cantidad" name="Paradas" radius={[0,4,4,0]}>
                      {motivosDist.map((_, i) => (
                ...
                <div className="dopd-no-data"><span>🔍</span><p>Sin datos de motivos</p></div>
              )}
            </div>
          ...
          {/* ══════════════════════════════════════════
              ...
              ══════════════════════════════════════════ */}
          <p className="dopd-section-title">📍 Análisis de Puntos de Actividad</p>
          <div className="dopd-charts-grid-3">
            ...
            <div className="dopd-card">
              <div className="dopd-card-header">
                ...
                <span className="dopd-card-badge">{puntosPorDia.length} días</span>
              </div>
                  ...
                  <BarChart data={volPorCatDia}>
                    ...
                    {cats.map(cat => (
                      <Bar
                        ...
                        fill={CAT_COLORS[cat]}
                        name={CAT_LABELS[cat]}
                        radius={cat === 'inspeccion' ? [4,4,0,0] : [0,0,0,0]}
                      />
                    ))}
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="dopd-no-data"><span>📦</span><p>Sin volumen registrado</p></div>
              )}
            </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
                    {categoriasDist.map(c => (
                      <div key={c.id} style={{
                        ...
                        <span style={{ fontSize: 13, fontWeight: 600, flex: 1, color: '#334155' }}>{c.nombre}</span>
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{c.valor} pts</span>
                        <span style={{ fontSize: 12, color: '#94a3b8' }}>{c.volumen} m³</span>
                      </div>
                    ))}
                  </div>
                </>
              ) : (
                <div className="dopd-no-data"><span>📍</span><p>Sin puntos en el período</p></div>
              )}
            </div>
            ...
            {puntosPorDia.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={puntosPorDia}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend wrapperStyle={{ fontSize: 12 }} />
                  <Line yAxisId="l" type="monotone" dataKey="puntos"  stroke="#2563eb" strokeWidth={2.5} dot={{ r: 4 }} name="Puntos" />
                  <Line yAxisId="r" type="monotone" dataKey="volumen" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 3 }} name="Volumen m³" />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <div className="dopd-no-data"><span>📈</span><p>Sin puntos en el período</p></div>
                    ...
                    <tbody>
                      {ultimasParadas.map(p => (
                        <tr key={p.id}>
                          ...
                          <td>
                            <span className="dopd-motivo-pill" title={p.motivoDisplay || p.motivoNombre}>
                              {p.motivoDisplay || p.motivoNombre || '—'}
                            </span>
                          </td>
                          <td>
                            {p.minutesParo != null
                              ? <span className="dopd-min-pill">⏱ {p.minutesParo} min</span>
                              : '—'}
                          </td>
                        </tr>
                      ))}
                  ...
                  <table className="dopd-table">
                    ...
                    <tbody>
                      {ultimosPuntos.map(pt => (
                        <tr key={pt.id}>
                          <td style={{ fontFamily: 'monospace', fontSize: 12 }}>{toDMY(pt.dia)}</td>
                          <td>
                            <span className={`dopd-cat-badge ${pt.categoria}`}>
                              {CAT_LABELS[pt.categoria] || pt.categoria || '—'}
                            </span>
                          </td>
                          <td style={{ maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: 500 }}>
                            {pt.itemDisplay || pt.itemNombre || '—'}
                          </td>
                          <td>
                            {pt.volumenM3 != null
                              ? <span className="dopd-vol-pill">📦 {Number(pt.volumenM3).toFixed(1)}</span>
                              : '—'}
                          </td>
                        </tr>
                      ))}
